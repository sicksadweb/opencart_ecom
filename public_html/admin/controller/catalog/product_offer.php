...
class ControllerCatalogProductOffer extends Controller {
	private $error = array();
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product->editOffer($this->request->get['product_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_catalog_product->deleteOffer($product_id);
			}
	...
	protected function getList() {
		...
		*/
		...
		$data['copy'] = $this->url->link('catalog/product_offer/copy', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/product_offer/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['enabled'] = $this->url->link('catalog/product_offer/enable', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['disabled'] = $this->url->link('catalog/product_offer/disable', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['add_filter_for_product'] = $this->url->link('catalog/product_offer/addFilterForProduct', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$product_total = $this->model_catalog_product->getTotalProductsOffer($filter_data);

		$results = $this->model_catalog_product->getProductsOffer($filter_data);
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_catalog_product->getProductSpecials($result['offer_id']);
			...
			$data['products'][] = array(
				...
				'href_shop'  => HTTP_CATALOG . 'index.php?route=product/product&product_id=' . $result['offer_id'],
				'edit'       => $this->url->link('catalog/product_offer/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $result['offer_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('catalog/product_offer', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/product_offer_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('catalog/product_offer/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/product_offer/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $this->request->get['product_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/product_offer', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_info = $this->model_catalog_product->getOffer($this->request->get['product_id']);
		...
		if (isset($this->request->post['product_description'])) {
			...
			$data['product_description'] = $this->model_catalog_product->getOfferDescriptions($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_link'])) {
			$product_links = $this->request->post['product_link'];
		} elseif (isset($product_info)) {
			$product_links = $this->model_catalog_product->getProductsLinks($this->request->get['product_id']);
		} else {
			$product_links = array();
		}
		...
		foreach ($product_links as $product_id) {
			$product_links_info = $this->model_catalog_product->getProductDescriptions($product_id);
						
			if ($product_links_info) {
				$data['product_links_info'][] = array(
					'product_id' => $product_links_info[1]['product_id'],
					'name'       => $product_links_info[1]['name']
				);
			}
		}
		...
		$this->response->setOutput($this->load->view('catalog/product_offer_form', $data));
	}
