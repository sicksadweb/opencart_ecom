...
class ModelCatalogCategory extends Model {
	...
	public function addCategory($data) {
		...
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category_views SET image = '" . $this->db->escape($data['image']) . "' WHERE views_id = '" . (int)$views_id . "'");
		}		

		// MySQL Hierarchical Data Closure Table Pattern
		$level = 0;

		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_views_path` WHERE views_id = '" . (int)$data['parent_id'] . "' ORDER BY `level` ASC");

		foreach ($query->rows as $result) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "category_views_path` SET `views_id` = '" . (int)$views_id . "', `path_id` = '" . (int)$result['path_id'] . "', `level` = '" . (int)$level . "'");

			$level++;
		}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "category_views_path` SET `views_id` = '" . (int)$views_id . "', `path_id` = '" . (int)$views_id . "', `level` = '" . (int)$level . "'");

		if (isset($data['category_filter'])) {
			foreach ($data['category_filter'] as $filter_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_views_filter SET views_id = '" . (int)$views_id . "', filter_id = '" . (int)$filter_id . "'");
			}
		}

		if (isset($data['category_store'])) {
			foreach ($data['category_store'] as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_views_to_store SET views_id = '" . (int)$views_id . "', store_id = '" . (int)$store_id . "'");
			}
		}
		...
		if (isset($data['category_seo_url'])) {
			foreach ($data['category_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (!empty($keyword)) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET store_id = '" . (int)$store_id . "', language_id = '" . (int)$language_id . "', query = 'category_id=" . (int)$views_id . "', keyword = '" . $this->db->escape($keyword) . "'");
					}
				}
			}
		}
		...
		if (isset($data['product_related'])) {
			foreach ($data['product_related'] as $related_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "view_related_wb SET view_id = '" . (int)$views_id . "', product_id = '" . (int)$related_id . "'");
			}
		}
	
		if (isset($data['article_related'])) {
			foreach ($data['article_related'] as $related_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "article_related_wb SET category_id = '" . (int)$views_id . "', article_id = '" . (int)$related_id . "'");
			}
		}
	...
	public function editCategory($category_id, $data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "category_views_description SET 
			language_id = '" . (int)$language_id . "', 
			name = '" . $this->db->escape($value['name']) . "', 
			description = '" . $this->db->escape($value['description']) . "', 
			meta_title = '" . $this->db->escape($value['name']) . "', 
			meta_h1 = '" . $this->db->escape($value['name']) . "', 
			meta_description = '" . $this->db->escape($value['name']) . "', 
			meta_keyword = '" . $this->db->escape($value['name']) . "'WHERE views_id = ". $category_id ."");
			
		}

		// MySQL Hierarchical Data Closure Table Pattern
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_views_path` WHERE path_id = '" . (int)$category_id . "' ORDER BY level ASC");

		if ($query->rows) {
			foreach ($query->rows as $category_path) {
				// Delete the path below the current one
				$this->db->query("DELETE FROM `" . DB_PREFIX . "category_views_path` WHERE views_id = '" . (int)$category_path['views_id'] . "' AND level < '" . (int)$category_path['level'] . "'");
				...
				// Get whats left of the nodes current path
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_views_path` WHERE views_id = '" . (int)$category_path['views_id'] . "' ORDER BY level ASC");
				...
				foreach ($path as $path_id) {
					$this->db->query("REPLACE INTO `" . DB_PREFIX . "category_views_path` SET views_id = '" . (int)$category_path['views_id'] . "', `path_id` = '" . (int)$path_id . "', level = '" . (int)$level . "'");
			...
			foreach ($query->rows as $result) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "category_views_path` SET views_id = '" . (int)$category_id . "', `path_id` = '" . (int)$result['path_id'] . "', level = '" . (int)$level . "'");

				$level++;
			}

			$this->db->query("REPLACE INTO `" . DB_PREFIX . "category_views_path` SET views_id = '" . (int)$category_id . "', `path_id` = '" . (int)$category_id . "', level = '" . (int)$level . "'");
		}

		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_filter WHERE views_id = '" . (int)$category_id . "'");

		if (isset($data['category_filter'])) {
			foreach ($data['category_filter'] as $filter_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_views_filter SET views_id = '" . (int)$category_id . "', filter_id = '" . (int)$filter_id . "'");
			}
		}

		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_to_store WHERE views_id = '" . (int)$category_id . "'");

		if (isset($data['category_store'])) {
			foreach ($data['category_store'] as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_views_to_store SET views_id = '" . (int)$category_id . "', store_id = '" . (int)$store_id . "'");
			}
		}
		...
		if (isset($data['product_related'])) {
			foreach ($data['product_related'] as $related_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "view_related_wb WHERE view_id = '" . (int)$category_id . "' AND product_id = '" . (int)$related_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "view_related_wb SET view_id = '" . (int)$category_id . "', product_id = '" . (int)$related_id . "'");
			}
		}
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_to_layout WHERE views_id = '" . (int)$category_id . "'");

		if (isset($data['category_layout'])) {
			foreach ($data['category_layout'] as $store_id => $layout_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_views_to_layout SET views_id = '" . (int)$category_id . "', store_id = '" . (int)$store_id . "', layout_id = '" . (int)$layout_id . "'");
			}
		}
	...
	public function deleteCategory($category_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_views_path WHERE path_id = '" . (int)$category_id . "'");

		foreach ($query->rows as $result) {
			$this->deleteCategory($result['views_id']);
		}		
		
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views WHERE views_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_description WHERE views_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_offers_description WHERE offers_id = '" . (int)$offers_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_filter WHERE views_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_to_store WHERE views_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_views_to_layout WHERE views_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "view_to_category WHERE view_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'category_id=" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "view_related_wb WHERE view_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "article_related_wb WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "coupon_category WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategory($category_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT GROUP_CONCAT(cd1.name ORDER BY level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') FROM
		 " . DB_PREFIX . "category_views_path cp 
		 LEFT JOIN " . DB_PREFIX . "category_views_description cd1 ON (cp.path_id = cd1.views_id AND cp.views_id != cp.path_id) 
	...
	public function getCategories($data = array()) {
		...
		$sql = "SELECT cp.views_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order, c1.noindex 
		FROM " . DB_PREFIX . "category_views_path cp 
		
		LEFT JOIN " . DB_PREFIX . "category_views c1 ON (cp.views_id = c1.views_id) 
		LEFT JOIN " . DB_PREFIX . "category_views c2 ON (cp.path_id = c2.views_id) 
		LEFT JOIN " . DB_PREFIX . "category_views_description cd1 ON (cp.path_id = cd1.views_id) 
		LEFT JOIN " . DB_PREFIX . "category_views_description cd2 ON (cp.views_id = cd2.views_id) 
	...
	public function getCategoryDescriptions($category_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_views_description WHERE views_id = '" . (int)$category_id . "'");
	...
	//New Function
	public function getTotalCategories() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category_views");
	...
	public function getCategoriesByParentId($parent_id = 0) {
		$query = $this->db->query("SELECT *, (SELECT COUNT(parent_id) FROM " . DB_PREFIX . "category_views WHERE parent_id = c.views_id) AS children FROM " . DB_PREFIX . "category_views c LEFT JOIN " . DB_PREFIX . "category_views_description cd ON (c.views_id = cd.views_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY c.sort_order, cd.name");
		return $query->rows;
	}
