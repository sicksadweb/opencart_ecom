...
class ControllerProductOffer extends Controller {
	...
	public function index() {
		...
		$product_info = $this->model_catalog_offer->getProduct($offer_id);
		...
		if ($product_info) {
			...
			$this->document->addLink($this->url->link('product/product', 'offer_id=' . $this->request->get['offer_id']), 'canonical');
			...
			$results = $this->model_catalog_offer->getProductImages($this->request->get['offer_id']);
			...
			$results = $this->model_catalog_offer->getProductAditionalImages($this->request->get['offer_id']);

			foreach ($results as $result) {
				$data['images_aditional'][] = array(
					'popup' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height')),
					'thumb' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height')),
					'name' => $result['name'],
					'alt' => $result['alt'],
			...
			$price =$this->model_catalog_offer->getProductPrice($this->request->get['offer_id'],$product_info['type_id'] );

			$data['price'] = $this->currency->format($this->tax->calculate($price['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']).'/ '.$product_info['abbr'];
print_r ($product_info['abbr']);
			...
			$discounts = $this->model_catalog_offer->getProductDiscounts($this->request->get['offer_id']);
			...
			foreach ($this->model_catalog_offer->getProductOptions($this->request->get['offer_id']) as $option) {
				...
				$data['options'][] = array(
					...
					'offer_option_value' => $product_option_value_data,
			...
			$data['share'] = $this->url->link('product/product', 'offer_id=' . (int)$this->request->get['offer_id']);
	...
	public function review() {
		...
		$review_total = $this->model_catalog_review->getTotalReviewsByProductId($this->request->get['offer_id']);

		$results = $this->model_catalog_review->getReviewsByProductId($this->request->get['offer_id'], ($page - 1) * 5, 5);
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('product/product/review', 'offer_id=' . $this->request->get['offer_id'] . '&page={page}');
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				$this->model_catalog_review->addReview($this->request->get['offer_id'], $this->request->post);
