...
class ControllerProductView extends Controller {
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				...
				$data['images'][] = array(
					...
					'price' => $this->currency->format($this->tax->calculate($result['price'] , $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']).'/ '.$result['abbr']
			...
			$discounts = $this->model_catalog_view->getProductDiscounts($this->request->get['view_id']);
			...
			foreach ($this->model_catalog_view->getProductOptions($this->request->get['view_id']) as $option) {
				...
				$data['options'][] = array(
					...
					'view_option_value' => $product_option_value_data,
			...
			$results = $this->model_catalog_view->getProductRelated($this->request->get['view_id']);
	...
	public function review() {
		...
		$review_total = $this->model_catalog_review->getTotalReviewsByProductId($this->request->get['view_id']);

		$results = $this->model_catalog_review->getReviewsByProductId($this->request->get['view_id'], ($page - 1) * 5, 5);
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('product/product/review', 'view_id=' . $this->request->get['view_id'] . '&page={page}');
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				$this->model_catalog_review->addReview($this->request->get['view_id'], $this->request->post);
