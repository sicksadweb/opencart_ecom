...
class ControllerProductviews extends Controller {
	public function index() {
		...
		$category_info = $this->model_catalog_views->getCategory($views_id);
		...
		if ($category_info) {
			...
			if ($page == 1) {
			    $this->document->addLink($this->url->link('product/views', 'path=' . $category_info['views_id']), 'canonical');
			} else {
				$this->document->addLink($this->url->link('product/views', 'path=' . $category_info['views_id'] . '&page='. $page), 'canonical');
			}
			
			if ($page > 1) {
			    $this->document->addLink($this->url->link('product/views', 'path=' . $category_info['views_id'] . (($page - 2) ? '&page='. ($page - 1) : '')), 'prev');
			}

			if ($limit && ceil($product_total / $limit) > $page) {
			    $this->document->addLink($this->url->link('product/views', 'path=' . $category_info['views_id'] . '&page='. ($page + 1)), 'next');
			}
	...
	//		$data['description'] = html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8');
			...
			$data['sorts'][] = array(
				'text'  => $this->language->get('text_default'),
				'value' => 'p.sort_order-ASC',
				'href'  => $this->url->link('product/views',  '&sort=p.sort_order&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('product/views',  '&sort=pd.name&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('product/views',  '&sort=pd.name&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_asc'),
				'value' => 'p.price-ASC',
				'href'  => $this->url->link('product/views',  '&sort=p.price&order=ASC' . $url)
			);
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/views', 'path=' . $url . '&limit=' . $value)
				);
			}
			...
			$pagination->page = $page;
			$pagination->limit = $limit;
			$pagination->url = $this->url->link('product/views',  $url . '&page={page}');
			...
			if ($page == 1) {
				...
				$this->document->addLink($this->url->link('product/views',  '&page='. $page), 'canonical');
			}
			...
			if ($limit && ceil($product_total / $limit) > $page) {
			    $this->document->addLink($this->url->link('product/views', '&page='. ($page + 1)), 'next');
			}
